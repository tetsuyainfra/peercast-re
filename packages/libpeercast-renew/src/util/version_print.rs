    ...
    F: Fn(&mut BTreeMap<&'static str, Option<&'static str>>) -> (),
{
    ...
    if output_as_json {
        let build_envs = build_envs.into_iter().filter_map(|(k, v)| v.map(|v| (k, v))).collect::<BTreeMap<_, _>>();
        ...
        let _pp = vergen_pretty::PrettyBuilder::default().env(build_envs).build()?.display(&mut stdout)?;
    }
