...
- 現在のpeercast-rootでは一つのPeerCast-Tracker(配信者のピアキャスト)から一つのチャンネル情報しか送信できません。
  - ずっと仕様を勘違していました。将来のアップデートでなんとかします
- PeerCastでは公開されているSessionIDを使ってコネクションの管理をしている。
  - 一番古いコネクションを最優先で保持し続けるが・・・何らかの理由でTCPのコネクションが不意にクライアント側のみで切断された場合、
    サーバー側にコネクションが残り(タイムアウトするまで)接続できなくなると思う（たまーに起きる不具合はこれではないか？）
    サーバー側でTCPのタイムアウトが来れば切断されればよいが・・・？
    - 公開情報でクライアントの管理をするのをあきらめていったん全部受け付けるが、送ってくるBCSTアトムで接続制御するのはどうか？
