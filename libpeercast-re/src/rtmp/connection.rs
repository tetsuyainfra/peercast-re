...
impl Connection {
        ...
        loop {
                ...
                .map_err(|x| format!("Connection {}: Failed to process bytes: {:?}", self.id, x))?
            {
                ...
                HandshakeProcessResult::Completed {
                    ...
                    stream.write_all(&response_bytes).await?;
                    tokio::spawn(self.start_connection_manager(stream, remaining_bytes));
                    return Ok(());
                }
            }
        }
    ...
    async fn start_connection_manager(
        ...
        // StreamManagerにコネクション開始のメッセージを送る
        if !send(&self.stream_manager_sender, message) {
            return Ok(());
        }

        // タスクを開始
        tokio::spawn(connection_reader(self.id, stream_reader, read_bytes_sender));
        ...
        // よーわからんけどセッションを作ってる・・・
        // https://docs.rs/rml_rtmp/latest/rml_rtmp/sessions/index.html
        // 一種のデータ保存場所で、サーバ・クライアント間でメッセージのやり取りを仲介している
        ...
        // セッションにハンドシェイク時に送信されて残っていたバッファを渡しておく
        ...
        loop {
            ...
            tokio::select! {
                // ↑のspawnしていた奴からデータが送られてくる
                message = read_bytes_receiver.recv() => {
                    match message {
                        None => break, // Noneが来た時はread_bytes_senderが閉じられた時ってことね・・・
                        Some(bytes) => {
                ...
                // StreamManagerからメッセージが送られてくるので処理する
    ...
    > {
        // println!("handle_connection_message: {:?}", &message);
        match message {
    ...
    fn handle_session_results(
        ...
        byte_writer: &mut mpsc::UnboundedSender<Packet>,
        ...
        for result in results.drain(..) {
            // println!("handle_session_results: {:?}", &result);
            match result {
                ...
                ServerSessionResult::RaisedEvent(event) => {
                    // 送られてきたデータはここの中でServerManagerに送られる
    ...
    fn handle_raised_event(
        ...
        // println!("handle_raised_event: {:?}", event);
        match event {
            ...
            ServerSessionEvent::PublishStreamRequested {
                ...
                match &self.state {
                    State::Connected { .. } => {
                        ...
                        let message: StreamManagerMessage = StreamManagerMessage::PublishRequest {
                            rtmp_app: app_name,
                            stream_key,
                            request_id,
                            connection_id: self.id,
            ...
            ServerSessionEvent::VideoDataReceived {
                ...
                // データはココからStreamManagerに送られる
                // println!(
                //     "handle_raised_event: {_app}/{_key} t:{timestamp:?} {} -> {:?}",
                //     data.len(),
                //     &self.state
                // );
                match &self.state {
