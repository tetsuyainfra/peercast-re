...
#[derive(Debug)]
pub struct ResultHttpPcpHandshake<S: Io> {
...
//
// HTTP requesのパース結果
pub struct IncomingHttpPcpHandshake<S: Io> {
    parts: Parts<S>,
    timeout: std::time::Duration,
}

impl<S: Io> IncomingHttpPcpHandshake<S> {
    pub fn new(
        cno: ConnectionNo,
        stream: S,
        remote: SocketAddr,
    ...
    pub async fn handshake(self) -> Result<ResultHttpPcpHandshake<S>, HandshakeError> {
        let Self {
            mut parts,
            timeout,
        } = self;
...
mod t {
    ...
    async fn test_identify_http() {
        ...
        let _h = tokio::spawn(async move {
            ...
            let handshake = IncomingHttpPcpHandshake::new(cno, stream, remote, buf.into(), None);
    ...
    async fn test_identify_http_test2() {
        ...
        let cno = ConnectionNo::new();
        let handshake = IncomingHttpPcpHandshake::new(cno, stream, remote, read_buf.into(), None);
        let r = handshake.handshake().await.unwrap();
        dbg!(&r);
    }
    ...
    async fn test_identify_protocol() {
        ...
        let cno = ConnectionNo::new();
        let handshake = IncomingHttpPcpHandshake::new(cno, server, remote, Some(read_buf), None);
        let r = handshake.handshake().await.unwrap();
        dbg!(&r);
    }
}
