    ...
    fn _start_channel_manager(&mut self) -> impl std::future::Future<Output = ()> + Send + '_ {
        {
            ...
            if let ChMgrStatus::Standby(rx) = old_manager_status {
                ...
                let channel_manager = manager::ChannelManager::new(rx, re_channel);
                return async move {
                    let _ = tokio::task::Builder::new().name(&name).spawn(channel_manager.start());
                };
            } else {
...
impl ReChannel {
    ...
    // ソースストリームを追加する
    pub async fn add_source_stream(&self, src_uri: Uri) -> anyhow::Result<()> {
        let _ = self.with_impl(|s| s.add_source_stream(src_uri))?;
        Ok(())
    }
...
impl ImplReChannel {
    ...
    fn add_source_stream(&self, src_uri: Uri) -> anyhow::Result<()> {
        let _ = self
            .sender
            .send(manager::Message::AddSourceStream(src_uri))
