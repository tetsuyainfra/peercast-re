pub mod ui;

pub mod api;

// for PeerCast Player's playlist/stream handling
pub mod peercast;

#[derive(utoipa::OpenApi)]
#[openapi(
    nest(
        (path = "/api/v1", api = api::v1::ApiV1),
    )
)]
pub struct ApiSetV1 {}

#[derive(utoipa::OpenApi)]
#[openapi(
    nest(
        (path = "/api/v2", api = api::v2::ApiV2),
    )
)]
pub struct ApiSetV2 {}

pub fn build_swagger() -> utoipa_swagger_ui::SwaggerUi {
    use utoipa::OpenApi;
    use utoipa_swagger_ui::SwaggerUi;
    use utoipa_swagger_ui::Url;

    let swagger_ui = SwaggerUi::new(crate::SWAGGER_PATH).urls(vec![
        (
            Url::with_primary("API v1", "/api-docs/openapi.json", true),
            ApiSetV1::openapi(), // to suppress unused warning
        ),
        (Url::new("API v2", "/api-docs/openapi2.json"), ApiSetV2::openapi()),
    ]);

    return swagger_ui;
}
